...
- **Add an N-gram anti-plagiarism hard gate** (spec §7/§10 — the SEO pass must run BOTH voice-damage AND 8-word anti-plagiarism vs `serp-bodies/`; the VOICE-DAMAGE replacement above only wired the voice-survival direction). Insert a new section:
  ...
  ```
- **Fix gate ordering:** ensure the `## Process` step list runs ALL four hard gates — voice-damage, n-gram anti-plagiarism, link-budget, originality — BEFORE the step that writes `04-seo.html` (the softskills source ran the link-budget validator AFTER the write; a gate that runs after the write gates nothing). Renumber the Process steps so "write 04-seo.html" is the LAST step, preceded by all gate checks; every gate section already says "do not write 04-seo.html [if it fires]" — the step order must match that.
- [ ] **Step 3: Verify**
```bash
cd D:/VP/ARTICLE_ENGINE/.claude/commands
