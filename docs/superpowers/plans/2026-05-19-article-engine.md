...
- [ ] **Step 1: Failing tests (every public method + each find/meta branch constrained — N5)** — `tests/test_wp_client.py`:
```python
...
class WPClient:
    ...
    def delete_post(self, pid):
        ...
        requests.delete(f"{self.base}/posts/{pid}",params={"force":True},
                        auth=self.auth,timeout=self.timeout)
```
- [ ] **Step 4: Run, verify pass** — PASS (12 — every public method + find-uid 200/404/5xx + find-slug empty/hit + read-meta present/absent; each bites a `return None`/constant/crash stub).
...
- [ ] **Step 1: Failing tests (incl. adversarial + gate-path coverage — N7)** — `tests/test_probe.py`:
```python
import responses
from scripts.lib.wp_client import WPClient
...
```
- [ ] **Step 4: Run, verify pass** — `python -m pytest tests/test_probe.py -v` → PASS (8 — meta-writable T/F, uid-roundtrip miss/match/wrong-id, detect-plugin rankmath/yoast/none; each bites a stub of the gate it guards).
...
# N10: the token swaps left ae-3's CourseCTA line pointing at `flagship_courses`,
# a key that exists only in the vendored seo/pillar-map.yaml — NOT in
# courses/trainingint.yaml (the spec's source of truth, which has course_url +
# secondary_courses). Repoint it deterministically to the real keys.
sed -i 's#read `courses/trainingint.yaml` for the assigned `flagship_courses.trainingint` and `trainingint` URLs)#read `courses/trainingint.yaml`: use the cluster `course_url` as the primary CourseCTA and `secondary_courses` for supporting links)#' ae-3-draft.md
```
- [ ] **Step 3: Verify (aligned to what the transforms actually produce)**
```bash
cd D:/VP/ARTICLE_ENGINE/.claude/commands
! grep -li "softskills\|how-to-speak-confidently\|pillar-map\.yaml\|src/content/blog\|2026-04-30-softskills\|flagship_courses\|/blog-[1-9]-\|# /blog-" ae-{1,2,3,4}-*.md && \
grep -q "serp-bodies" ae-2-serp-analyze.md && \
grep -q "Article Engine pipeline (trainingint.com)" ae-1-keyword-research.md && \
grep -q "use the cluster \`course_url\` as the primary CourseCTA" ae-3-draft.md && echo "ae-1..4 OK"
```
Expected: prints `ae-1..4 OK` (the leading `!` makes the first grep succeed only if NOTHING matches — every softskills/fixture/old-path/`flagship_courses`/stale-`/blog-N`-command string gone; and the N10 CourseCTA repoint is present).
...
- **Add an N-gram anti-plagiarism hard gate** (spec §7/§10 — the SEO pass must run BOTH voice-damage AND 8-word anti-plagiarism vs `serp-bodies/`; the VOICE-DAMAGE replacement above only wired the voice-survival direction). Insert a new section:
  ```
  ...
  ```
- **Fix gate ordering AND the write-vs-read coherence (N12):** the gate Python one-liners all do `open('…/_draft/04-seo.html').read()` — they read the file from disk — so the candidate MUST be written before the gates run, yet a gate-failing candidate must never be left as the final artifact. Restructure `## Process` as: step "assemble + **write the candidate `04-seo.html`**" → then the four hard gates (voice-damage, n-gram anti-plagiarism, link-budget, originality), each reading that file → on ANY gate failure: **delete `04-seo.html`** (so a voice-damaged/plagiarised/over-budget candidate is never left behind), show the diff/violation, STOP → only if ALL four gates pass does `04-seo.html` stand (it is already on disk). The per-gate sections' wording must read "delete the candidate `04-seo.html` and STOP" (NOT "do not write" — it is written first by design); the final step is "all four gates passed → `04-seo.html` stands; then run the 80-item checklist + write `_audit/seo-checklist.md`". This keeps spec §7's "refuse to overwrite if voice drifted" guarantee (a failed candidate is removed) while making the disk-reading gate commands actually runnable.
- [ ] **Step 3: Verify**
```bash
cd D:/VP/ARTICLE_ENGINE/.claude/commands
...
grep -q "overlap_8gram" ae-6-seo-pass.md && \
grep -q "04-seo.html" ae-6-seo-pass.md && \
! grep -qi "npx tsx\|auto-emitted\|softskills\|04-seo\.md\b\|BlogLayout\|/blog-[1-9]-\|# /blog-" ae-6-seo-pass.md && echo "ae-6 OK"
```
Expected: prints `ae-6 OK` (confirms TS validator, BlogLayout, softskills, stale `/blog-N` command names, `.md` output all gone; all four Python gates incl. `overlap_8gram` anti-plagiarism + sibling/img placeholders + HTML output present).
...
grep -q "_yoast_wpseo_title" ae-8-publish.md && \
grep -q "rank_math_title" ae-8-publish.md && \
grep -q "does not exist yet, treat it as empty\|does not exist" ae-8-publish.md && \
grep -q "status: proposed" ae-batch.md && \
grep -q "Slugs run concurrently" ae-batch.md && \
! grep -qi "lighthouse\|coolify\|index.mdx\|npm run build\|GSC indexing" ae-8-publish.md && \
echo "ae-8 + ae-batch OK"
```
Expected: prints `ae-8 + ae-batch OK` (now also asserts the N13 input-derivation: seo_meta plugin-key mapping + status.yaml bootstrap + ae-batch parallelism are present).
...
- wp_client N5 (`test_find_slug_returns_id_when_found`, `test_update_post_returns_id`, `test_read_post_meta_returns_value`/`_missing_key_returns_none`, `test_find_uid_server_error_raises`, `test_me_returns_dict`, `test_upload_media_returns_id`, `test_delete_post_sends_force`): a `return None`/constant/crash stub of the gate-critical methods fails each — closes the 6/9-untested-method blind spot on the idempotency guard + probe-meta decision. ✅
